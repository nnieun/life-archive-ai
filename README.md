...
사람의 기억은 정확한 날짜나 완성된 문장으로 남아 있지 않습니다.
“초등학교 때였나?”, “친구랑 공원에 갔던 것 같은데”처럼 불완전하고 일상적인 말로 남는 경우가 더 많습니다.

**기억함**은 이런 생애 기록을 구조화된 기억으로 저장하고, 필요한 기억을 다시 찾아 질문에 답하거나 타임라인과 자서전으로 만들어 주는 Memory-Centric RAG 프로젝트입니다.

모든 AI 생성 결과에는 원본 대화 기록의 위치를 함께 표시합니다. 확인할 수 없는 내용은 임의로 만들어내지 않고, 근거가 부족하면 답변을 거절하도록 설계했습니다.

## 1. 프로젝트 개요

일반적인 문서 기반 RAG가 문서의 일부를 찾아 답변하는 데 집중한다면, 기억함은 대화 기록에서 사건 단위의 기억을 추출해 장기적으로 관리하는 데 초점을 둡니다.
